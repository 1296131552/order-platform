...
CREATE TABLE `t_carrier` (
  ...
  -- 道路运输经营许可证号,承运商特有资质
  -- 默认空字符串表示未填写,避免使用NULL防止索引失效和简化查询逻辑
  `road_transport_license_no` VARCHAR(50) NOT NULL DEFAULT '' COMMENT '道路运输经营许可证号:默认空字符串表示未填写',
  ...
  -- 企业网址
  `website` VARCHAR(200) NOT NULL DEFAULT '' COMMENT '企业网址',
  ...
  -- ========== 状态信息 ==========
  -- 承运商状态:ACTIVE(启用)、INACTIVE(停用)、SUSPENDED(暂停)
  -- 用于控制承运商是否可用,停用后不能关联新发运批次
  -- 默认ACTIVE表示启用状态,避免使用NULL防止索引失效和简化查询逻辑
  `status` VARCHAR(20) NOT NULL DEFAULT 'ACTIVE' COMMENT '承运商状态:默认ACTIVE表示启用状态',
  
  -- ========== 评级信息 ==========
  -- 承运商等级,如:A级、B级、C级,用于承运商分类管理
  -- 默认空字符串表示未评级,避免使用NULL防止索引失效和简化查询逻辑
  `carrier_level` VARCHAR(10) NOT NULL DEFAULT '' COMMENT '承运商等级:默认空字符串表示未评级',
  -- 承运商评分,0-100分,用于承运商评价
  -- 使用NULL表示未评分,必须区分"未评分"和"已评分为0分"的业务含义,因此允许使用NULL
  `carrier_score` DECIMAL(5, 2) NULL DEFAULT NULL COMMENT '承运商评分:NULL表示未评分',
  
  -- ========== 运输能力信息 ==========
  -- 运输范围,如:全国、华东、华北等
  `transport_scope` VARCHAR(100) NOT NULL DEFAULT '' COMMENT '运输范围',
  -- 主要运输方式,如:公路运输、铁路运输、航空运输等
  `main_transport_mode` VARCHAR(50) NOT NULL DEFAULT '' COMMENT '主要运输方式',
  ...
  -- ========== 公共字段 ==========
  -- 创建时间(精确到毫秒),记录承运商创建的时间
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
  -- 创建人ID,记录承运商的创建人,外键关联t_user表。-1表示系统自动操作,0及以上表示用户ID
  -- 默认-1表示系统创建,避免使用NULL防止空指针问题和简化查询逻辑
  `created_by` BIGINT NOT NULL DEFAULT -1 COMMENT '创建人ID:默认-1表示系统创建',
  -- 更新时间(精确到毫秒),记录承运商信息最后更新的时间
  `updated_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
  -- 更新人ID,记录承运商信息的最后更新人,外键关联t_user表。-1表示系统自动操作,0及以上表示用户ID
  ...
  CONSTRAINT `ck_carrier_contact_phone` CHECK (`contact_phone` = '' OR `contact_phone` REGEXP '^\\+?[0-9]{7,20}$'),
  CONSTRAINT `ck_carrier_phone` CHECK (`phone` = '' OR `phone` REGEXP '^\\+?[0-9]{7,20}$'),
  ...
  -- 承运商编码唯一索引
  UNIQUE KEY `uk_carrier_code` (`carrier_code`) COMMENT '承运商编码唯一索引',
  
  -- 统一社会信用代码唯一索引(用于企业身份识别)
  UNIQUE KEY `uk_carrier_unified_social_credit_code` (`unified_social_credit_code`) COMMENT '统一社会信用代码唯一索引',
  
  -- 承运商名称索引(用于模糊查询)
  KEY `idx_carrier_name` (`carrier_name`) COMMENT '承运商名称索引',
  
  -- 状态+软删除联合索引,用于查询启用的承运商
  KEY `idx_carrier_status` (`status`, `is_deleted`) COMMENT '状态软删除联合索引',
  
  -- 承运商等级+状态+软删除联合索引,用于按等级查询承运商
  KEY `idx_carrier_level_status` (`carrier_level`, `status`, `is_deleted`) COMMENT '承运商等级状态联合索引',
  
  -- 创建时间+软删除联合索引,用于时间排序和统计查询
  KEY `idx_carrier_created` (`created_at`, `is_deleted`) COMMENT '创建时间软删除联合索引'
  
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci 
COMMENT='承运商表:合作方-承运商信息表,记录承运商基本信息、企业信息、联系方式、地址、状态、运输能力等。'
  '承运商关联发运批次,可关联附件(资质文件)和履约统计。';
