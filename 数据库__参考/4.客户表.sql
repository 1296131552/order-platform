-- ============================================================
-- 客户表 (t_customer)
-- 说明:客户信息表,记录客户基本信息、联系方式、地址等
-- 关系:Customer 1:N Order (客户关联订单)
-- ============================================================

CREATE TABLE `t_customer` (
  -- ========== 主键 ==========
  -- 客户ID,主键,自增。每条客户记录的唯一标识
  -- 注意:目前使用自增ID,若使用分布式架构应改为雪花ID
  `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '客户ID',
  
  -- ========== 基本信息 ==========
  -- 客户编码,业务唯一标识,如:CUST001
  `customer_code` VARCHAR(50) NOT NULL COMMENT '客户编码',
  -- 客户名称,企业全称
  `customer_name` VARCHAR(200) NOT NULL COMMENT '客户名称',
  -- 客户简称,用于展示
  `customer_short_name` VARCHAR(100) NOT NULL DEFAULT '' COMMENT '客户简称',
  ...
  -- ========== 状态信息 ==========
  -- 客户状态:ACTIVE(启用)、INACTIVE(停用)、SUSPENDED(暂停)
  -- 用于控制客户是否可用,停用后不能关联新订单
  -- 默认ACTIVE表示启用状态,避免使用NULL防止索引失效和简化查询逻辑
  `status` VARCHAR(20) NOT NULL DEFAULT 'ACTIVE' COMMENT '客户状态:默认ACTIVE表示启用状态',
  ...
  -- ========== 公共字段 ==========
  -- 创建时间(精确到毫秒),记录客户创建的时间
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
  -- 创建人ID,记录客户的创建人,外键关联t_user表。-1表示系统自动操作,0及以上表示用户ID
  -- 默认-1表示系统创建,避免使用NULL防止空指针问题和简化查询逻辑
  `created_by` BIGINT NOT NULL DEFAULT -1 COMMENT '创建人ID:默认-1表示系统创建',
  -- 更新时间(精确到毫秒),记录客户信息最后更新的时间
  `updated_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
  -- 更新人ID,记录客户信息的最后更新人,外键关联t_user表。-1表示系统自动操作,0及以上表示用户ID
  ...
  CONSTRAINT `ck_customer_contact_phone` CHECK (`contact_phone` = '' OR `contact_phone` REGEXP '^\\+?[0-9]{7,20}$'),
  CONSTRAINT `ck_customer_phone` CHECK (`phone` = '' OR `phone` REGEXP '^\\+?[0-9]{7,20}$'),
  ...
  -- 客户编码唯一索引
  UNIQUE KEY `uk_customer_code` (`customer_code`, `is_deleted`) COMMENT '客户编码唯一索引',
  
  -- 统一社会信用代码唯一索引(用于企业身份识别,如果填写)
  -- 注意:由于统一社会信用代码可能为空,无法使用唯一约束,业务层需保证唯一性
  KEY `idx_customer_unified_social_credit_code` (`unified_social_credit_code`, `is_deleted`) COMMENT '统一社会信用代码索引',
  
  -- 客户名称索引(用于模糊查询)
  KEY `idx_customer_name` (`customer_name`, `is_deleted`) COMMENT '客户名称索引',
  
  -- 状态+软删除联合索引,用于查询启用的客户
  KEY `idx_customer_status` (`status`, `is_deleted`) COMMENT '状态软删除联合索引',
  
  -- 创建时间+软删除联合索引,用于时间排序和统计查询
  KEY `idx_customer_created` (`created_at`, `is_deleted`) COMMENT '创建时间软删除联合索引'
  
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci 
COMMENT='客户表:客户信息表,记录客户基本信息、企业信息、联系方式、地址、状态等。'
  '客户通过订单关联业务,一个客户可以关联多个订单。客户信息可以关联附件(合同、需求单等)。';
