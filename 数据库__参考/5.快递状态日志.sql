-- ============================================================
-- 快递单状态日志表 (t_shipment_line_status_log)
-- 说明:记录快递单全生命周期的状态变更历史,支持业务时间线和审计追溯
-- ============================================================

CREATE TABLE `t_shipment_line_status_log` (
  -- ========== 主键 ==========
  ...
  -- 关系:ShipmentLineStatusLog N:1 StatusDict (状态日志引用状态字典)
  -- 变更前的快递单状态字典ID。外键关联t_status_dict表。初始创建时from_status_dict_id为NULL(表示无前序状态)
  ...
  -- 变更后的快递单状态字典ID。外键关联t_status_dict表。必须关联状态字典,保证状态定义统一
  `to_status_dict_id` BIGINT NOT NULL COMMENT '变更后状态字典ID',
  -- 变更前的快递单状态代码(冗余字段,便于查询和展示)。从StatusDict表中同步,避免频繁JOIN查询
  ...
  -- 变更后的快递单状态代码(冗余字段,便于查询和展示)。从StatusDict表中同步,避免频繁JOIN查询
  -- 注意:此字段应与to_status_dict_id关联的状态字典记录中的status_code保持一致
  `to_status` VARCHAR(50) NOT NULL COMMENT '变更后状态代码(冗余字段)',
  
  -- ========== 业务节点信息 ==========
  -- 业务节点代码,用于业务时间线展示。命名规范示例:shipment_line_created(快递单创建)、pickup_started(开始提货)、
  -- pickup_completed(提货完成)、in_transit(运输中)、delivery_arrived(到货)、receipt_started(开始签收)、receipt_completed(签收完成)
  -- 空字符串表示无具体业务节点
  -- TODO: 业务节点名称还需后续确认
  ...
  -- 业务阶段枚举:SHIPMENT_LINE_INIT(快递单初始化)、PICKUP(提货)、TRANSPORT(运输)、DELIVERY(到货)、RECEIPT(签收)
  -- 标识快递单生命周期的大阶段,用于按阶段查询和统计
  `business_stage` VARCHAR(30) NOT NULL DEFAULT '' COMMENT '业务阶段',
  
  -- ========== 操作人信息 ==========
  -- 操作人ID,外键关联t_user表。-1表示系统自动操作,0及以上表示用户ID
  -- 注意:与快递单表的created_by/updated_by保持一致,使用-1作为系统操作的默认值
  ...
  -- ========== 变更原因和备注 ==========
  -- 状态变更原因/操作说明,用于业务追溯和问题排查。示例:"开始提货"、"运输途中异常"、"到货"、"签收完成"
  ...
  -- 示例:receipt_detail(签收明细)、exception(异常记录)、carrier(承运商)
  `related_business_type` VARCHAR(30) NOT NULL DEFAULT '' COMMENT '关联业务类型',
  -- 关联业务对象ID,如:签收明细ID、异常记录ID等。0表示无关联业务对象
  ...
  -- CONSTRAINT `fk_shipment_line_status_log_from_status_dict` FOREIGN KEY (`from_status_dict_id`) REFERENCES `t_status_dict` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE,
  -- CONSTRAINT `fk_shipment_line_status_log_to_status_dict` FOREIGN KEY (`to_status_dict_id`) REFERENCES `t_status_dict` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE,
  
  -- ========== 索引设计说明 ==========
  -- 索引设计原则:只保留核心高频查询场景的索引,避免过度索引导致写入性能下降
  -- InnoDB联合索引遵循最左前缀原则,单字段索引被联合索引覆盖时需删除以避免冗余
  
  -- 核心查询:快递单时间线查询(最常用,高频查询)
  -- 查询场景:查询某个快递单的所有状态变更记录,按时间顺序展示(业务时间线)
  -- 索引字段:shipment_line_id(定位快递单)+ is_deleted(软删除筛选)+ operate_time(时间排序)
  KEY `idx_shipment_line_status_log_shipment_line_time` (`shipment_line_id`, `is_deleted`, `operate_time`) COMMENT '快递单时间线查询索引'
  
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci 
COMMENT='快递单状态日志表:记录快递单全生命周期的状态变更历史,支持业务时间线展示、操作审计、状态追溯等功能。'
  '每条记录代表一次快递单状态的变更操作,包含变更前后的状态信息、操作人信息、业务节点信息、关联业务对象等完整信息。'
