<template>
  <div class="carrier-list">
    ...
    <el-card class="table-card" shadow="never">
      ...
      >
        <el-table-column prop="carrierNo" label="承运商编码" width="140" />
...
import { getCarrierList, deleteCarrier, getCarrierStatistics, activateCarrier, deactivateCarrier, type CarrierQueryParams } from '@/api/carrier'
...
const searchItems = [
  ...
  { prop: 'carrierNo', label: '承运商编码', type: 'input' },
...
const handleDeactivate = async (row: any) => {
  ElMessageBox.confirm(`确定要停用承运商 "${row.name}" 吗？`, '提示', {
    type: 'warning'
  }).then(async () => {
    try {
      await deactivateCarrier(row.id)
