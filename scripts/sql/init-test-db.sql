...
CREATE TABLE `t_user` (
  ...
  `audit_status` VARCHAR(30) NOT NULL DEFAULT 'NONE' COMMENT '审核状态:NONE-无需审核,PENDING-待审核,APPROVED-已通过,REJECTED-已拒绝',
  `audit_remark` VARCHAR(500) NOT NULL DEFAULT '' COMMENT '审核备注',
  `audit_by` BIGINT NOT NULL DEFAULT -1 COMMENT '审核人ID',
